...
**Решение:** серверный геокодинг через **Nominatim (OpenStreetMap)**.

- Бесплатный, без API-ключа
- Ограничение: 1 запрос/сек, обязательный User-Agent
- Fallback: если покрытие РФ окажется слабым — переключение на DaData (10k бесплатных/день)

**При неудаче геокодирования:**

При получении цены курьерской доставки (до оформления) — вернуть ошибку клиенту («не удалось определить адрес»).
При создании заказа (webhook от Тильды) — отправить email с данными заказа в ЧП-виде, дальше ручной режим (общая система ошибок, см. plan-1 п.1.6).

Реализация: `src/utils/geocode.ts` (общий с этапом 1).
